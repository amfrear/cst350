...
namespace ButtonGrid.Controllers
{
    public class ButtonController : Controller
    {
        ...
        public IActionResult RightClickShowOneButton(int id)
        {
            ButtonModel button = buttons.FirstOrDefault(b => b.Id == id);
            if (button != null)
            {
                button.ButtonState = 0; // Reset button state to 0
                button.ButtonImage = $"/img/{buttonImages[button.ButtonState]}"; // Ensure path consistency
            }
            return PartialView("_Button", button);
        }
    }
