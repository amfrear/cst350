...
namespace ProductsApp.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<IEnumerable<ProductViewModel>> SearchForProducts(SearchFor searchCriteria)
        {
            ...
            {
                if (searchCriteria.InTitle)
                {
                    var nameMatches = await _productDAO.SearchForProductsByName(searchCriteria.SearchTerm);
                    products.AddRange(nameMatches);
                }

                if (searchCriteria.InDescription)
                {
                    var descriptionMatches = await _productDAO.SearchForProductsByDescription(searchCriteria.SearchTerm);
